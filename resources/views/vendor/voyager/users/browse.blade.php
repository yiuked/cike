                ...
                <div class="panel panel-bordered">
                    <div class="panel-body">
                        <table id="dataTable" class="table table-hover">
                            ...
                            @foreach($dataTypeContent as $data)
                                <tr>
                                    ...
                                    <td>
                                        <img src="@if( strpos($data->avatar, 'http://') === false && strpos($data->avatar, 'https://') === false){{ Voyager::image( $data->avatar ) }}@else{{ $data->avatar }}@endif" style="width:40px;border-radius: 20px;">
                                    </td>
                        ...
                        @if (isset($dataType->server_side) && $dataType->server_side)
                            <div class="pull-left">
                                <div role="status" class="show-res" aria-live="polite">显示 {{ $dataTypeContent->firstItem() }} 到 {{ $dataTypeContent->lastItem() }} 共 {{ $dataTypeContent->total() }} 条记录</div>
                            </div>
                ...
                <div class="modal-footer">
                    <form action="{{ route('voyager.'.$dataType->slug.'.index') }}" id="delete_form" method="POST">
                        ...
                        <input type="submit" class="btn btn-danger pull-right delete-confirm"
                                 value="是的，删除这个{{ $dataType->display_name_singular }}">
                    </form>
